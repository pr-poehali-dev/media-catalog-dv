...
function statTable(b: PdfBuilder, community: Community) {
  const doc = b.doc;
  ...
  community.socials.forEach((s) => {
    ...
    doc.text(s.subscribers ? fmtSubs(s.subscribers) : '—', cx + 3, b.y + 5.4);
    cx += colW[1];
...
function priceTable(b: PdfBuilder, community: Community) {
  const doc = b.doc;
  community.prices.forEach((p) => {
    b.needSpace(7);
...
export function renderCommunity(b: PdfBuilder, community: Community, withHeaderBlock = true) {
  const doc = b.doc;
  ...
  if (withHeaderBlock) {
    ...
    doc.text(`${community.city.toUpperCase()} · ${community.category.toUpperCase()}`, PAGE.mx + 6, b.y + 8, { charSpace: 0.3 });
    b.font('bold', 16, COLORS.milk);
    doc.text(community.name, PAGE.mx + 6, b.y + 16);
    b.font('normal', 8, '#C8C2B8');
    const desc = doc.splitTextToSize(community.fullDescription || community.description, CONTENT_W - 12);
    doc.text(desc.slice(0, 2), PAGE.mx + 6, b.y + 22);
    b.y += blockH + 5;
  } else {
    b.eyebrow(`${community.city} · ${community.category}`);
    b.heading(community.name, 15);
    b.gap(1);
    b.paragraph(community.fullDescription || community.description, 9.5, COLORS.muted);
    b.gap(3);
  }

  if (community.audienceCharts) {
    b.eyebrow('Аудитория');
    const a = community.audienceCharts;
    ...
    b.gap(2);
  } else if (community.audience?.length) {
    b.eyebrow('Аудитория');
    b.bulletList(community.audience);
    b.gap(2);
  }

  b.eyebrow('Соцсети и статистика');
  statTable(b, community);

  b.eyebrow('Форматы размещения');
  b.tags(community.formats);
  b.gap(1);

  if (community.prices?.length) {
    b.eyebrow('Стоимость');
    priceTable(b, community);
  }
