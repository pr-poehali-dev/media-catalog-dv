...
const LEGAL_CONTENT: Record<string, { title: string; content: string }> = {
  ...
  cookies: {
    ...
    content: `## Что такое Cookie?\n\nCookie — небольшие текстовые файлы, сохраняемые в браузере при посещении сайта.\n\n## Какие Cookie используем?\n\n**Технические Cookie** — необходимы для работы сайта. Не отключаются.\n\n**Аналитические Cookie** — Яндекс.Метрика. Помогают анализировать посещаемость. Загружаются только после вашего согласия.\n\n## Управление\n\nПри первом посещении вы выбираете «Принять» или «Отклонить». Если вы нажали «Отклонить», аналитические скрипты не загружаются, а отказ сохраняется и действует при последующих визитах. Технические Cookie продолжают работать, они необходимы для работы сайта.\n\nИзменить выбор можно в любой момент по ссылке «Настройки cookie» в нижней части сайта, а также через настройки браузера.\n\n## Согласие\n\nАналитические Cookie используются только при явно выраженном согласии. Продолжение просмотра сайта не отменяет ранее выбранный отказ.`,
  },
  'ad-marking': {
    ...
    <div>
      <section className="bg-[#0A0A0A] text-[#FBF8F3] pattern-dark hero-pad-top pb-16">
        <div className="pattern-content max-w-7xl mx-auto px-6">
          ...
          <h1 className="page-hero-title text-[#FBF8F3]">{page.title}</h1>
        </div>
      </section>
