...
import { PLATFORMS, SOCIALS, SocialNet } from '@/data/political-data';
      ...
      <div>
          ...
          <div className="max-w-7xl mx-auto px-6 max-w-3xl">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-[#E8E2D8] mb-10">
              {(['vk', 'telegram', 'max', 'tiktok'] as const).map((key) => {
                const s = SOCIALS[key];
                const paths: Record<string, string> = { vk: '/political/socials/vk', telegram: '/political/socials/telegram', max: '/political/socials/max', tiktok: '/political/socials/tiktok' };
                return (
        ...
        <section className="bg-[#0A0A0A] pattern-dark py-14 reveal">
          <div className="pattern-content max-w-7xl mx-auto px-6 text-center text-[#FBF8F3]">
            ...
            <Link to="/political/contacts" className="btn-carmine">Получить консультацию</Link>
          </div>
        </section>
      </div>
    );
